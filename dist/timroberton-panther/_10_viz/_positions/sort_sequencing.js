...
export function sortSequencing(nodes, columns, nodeCoordMap, s) {
    ...
    const tieBreakerWeights = nodes.reduce((map, node, i) => {
        map[node.id] = i;
        return map;
    }, {});
    const colHeights = [];
    ...
    columns.forEach((colInfo, i) => {
        ...
        colInfo.nodeIds
            .filter((nodeId) => !nodeCoordMap[nodeId].isConnecting)
            .forEach((nodeId) => {
    ...
    columns.forEach((colInfo) => {
        ...
        colInfo.nodeIds.sort((b, c) => {
            ...
            if (nodeCoordMap[b].isConnecting && nodeCoordMap[c].isConnecting) {
                return (tieBreakerWeights[nodeCoordMap[b].endFrIfConnecting] -
                    tieBreakerWeights[nodeCoordMap[c].endFrIfConnecting]);
            }
