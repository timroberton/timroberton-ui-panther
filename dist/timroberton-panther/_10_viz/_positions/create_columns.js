...
export function createColumnsAndUpdateTileCoordMap(tiles, nodes, tileCoordMap, s) {
    ...
    const columns = new Array(maxLayer + 1)
        .fill(0)
        .map(() => {
        return {
            nodeIds: [],
    ...
    for (let i = 0; i < tiles.length; i++) {
        ...
        tc.finalToLayer = tiles[i + 1]
            ? tiles[i + 1].initialFromLayer - 1
            : maxLayer;
    }
    // Add joins
