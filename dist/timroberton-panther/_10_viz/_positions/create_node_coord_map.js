...
export function newNodeCoords(w, h, isConnecting, layer, endFrIfConnecting, endToIfConnecting) {
    return {
        w,
        h,
        isConnecting,
...
export function createEdgeCoordMap(edges) {
...
export function createTileCoordMap(tiles) {
    return tiles.reduce((map, edge) => {
        map[edge.id] = {
            finalFromLayer: -1,
            finalToLayer: -1,
            joinL: false,
...
export function addAndSortNodeJoins(nodes, edges, nodeCoordMap, edgeCoordMap) {
    edges.forEach((a) => {
        const nodeChain = edgeCoordMap[a.id].nodeChain;
        assert(nodeChain.length >= 2);
        ...
        if (segmentDirection === SegmentDirection.LeftToRight) {
            nodeCoordMap[a.fr].joinsR.push({
                incomingOutgoing: IncomingOutgoing.Outgoing,
                nextNodeChainIdForSorting: nextFr,
                finalOrder: -1,
            });
            nodeCoordMap[a.to].joinsL.push({
                incomingOutgoing: IncomingOutgoing.Incoming,
                nextNodeChainIdForSorting: nextTo,
                finalOrder: -1,
            });
        }
        if (segmentDirection === SegmentDirection.RightToLeft) {
            nodeCoordMap[a.fr].joinsL.push({
                incomingOutgoing: IncomingOutgoing.Outgoing,
                nextNodeChainIdForSorting: nextFr,
                finalOrder: -1,
            });
            nodeCoordMap[a.to].joinsR.push({
                incomingOutgoing: IncomingOutgoing.Incoming,
                nextNodeChainIdForSorting: nextTo,
                finalOrder: -1,
            });
        }
        if (segmentDirection === SegmentDirection.VerticalAroundDown ||
            segmentDirection === SegmentDirection.VerticalAroundUp) {
            nodeCoordMap[a.fr].joinsL.push({
                incomingOutgoing: IncomingOutgoing.Outgoing,
                nextNodeChainIdForSorting: nextFr,
                finalOrder: -1,
        ...
        if (segmentDirection === SegmentDirection.ImmediateDown) {
            nodeCoordMap[a.fr].joinsB.unshift({
                other: a.to,
                incomingOutgoing: IncomingOutgoing.Outgoing,
            }); // Use unshift so down is always on left
            nodeCoordMap[a.to].joinsT.unshift({
                other: a.fr,
                incomingOutgoing: IncomingOutgoing.Incoming,
            }); // Use unshift so down is always on left
        }
        if (segmentDirection === SegmentDirection.ImmediateUp) {
            nodeCoordMap[a.fr].joinsT.push({
                other: a.to,
                incomingOutgoing: IncomingOutgoing.Outgoing,
            });
            nodeCoordMap[a.to].joinsB.push({
                other: a.fr,
                incomingOutgoing: IncomingOutgoing.Incoming,
            });
        }
    ...
    nodes.forEach((a) => {
        ...
        nodeCoordMap[a.id].joinsL.sort((b, c) => {
            ...
            if (bDir === SegmentDirection.ImmediateUp ||
                bDir === SegmentDirection.ImmediateDown ||
                cDir === SegmentDirection.ImmediateUp ||
                cDir === SegmentDirection.ImmediateDown) {
                throw new Error("Not possible");
            }
            if (bDir === SegmentDirection.VerticalAroundDown &&
                cDir === SegmentDirection.VerticalAroundDown) {
                return smallestSequenceGoesLast;
            }
            if (bDir === SegmentDirection.VerticalAroundUp &&
                cDir === SegmentDirection.VerticalAroundUp) {
                return smallestSequenceGoesLast;
            }
            if (bDir === SegmentDirection.VerticalAroundUp &&
                cDir === SegmentDirection.VerticalAroundDown) {
        ...
        nodeCoordMap[a.id].joinsR.sort((b, c) => {
            ...
            return (nodeCoordMap[b.nextNodeChainIdForSorting].sequence -
                nodeCoordMap[c.nextNodeChainIdForSorting].sequence);
        });
...
export function updateNodeCoordMapWithYOffsets(nodes, nodeCoordMap, s) {
    nodes.forEach((a) => {
...
export function updateNodeCoordMapWithTrackMaps(columns, nodes, nodeCoordMap, s) {
    columns.forEach((colInfo) => {
        colInfo.segmentsNormalToTheRight.forEach((seg) => {
            nodeCoordMap[seg.left].trackMapXR_forLeftNodes[getKeyInOutNext(seg.horizontalDirection === HorizontalDirection.LeftToRight
                ? IncomingOutgoing.Outgoing
                : IncomingOutgoing.Incoming, seg.right)] = seg.track;
        });
    ...
    nodes.forEach((node) => {
        ...
        if (nTop === 2) {
            ...
            if (j1.incomingOutgoing === IncomingOutgoing.Incoming) {
                nodeCoordMap[node.id].immediateOffsetMapIncoming[j1.other] =
                    0 - halfSep;
                nodeCoordMap[node.id].immediateOffsetMapOutgoing[j2.other] = halfSep;
            }
            else {
                nodeCoordMap[node.id].immediateOffsetMapOutgoing[j1.other] =
                    0 - halfSep;
