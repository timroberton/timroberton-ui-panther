export function updateHorizontalPositions(columns, nodes, nodeCoordMap, s) {
    nodes.forEach((node) => {
    ...
    columns.forEach((c, i) => {
        ...
        //
        const extraPaddingForTracksNormalR = s.SHOW_TILES && c.hasTracksNormalR && c.tileBreakR
            ? s.TILE_HORIZONTAL_PADDING
            : 0;
        const widthTracksNormalR = c.hasTracksNormalR
            ? (c.numberTracksNormalR - 1) * s.EDGE_TO_EDGE_HORIZONTAL_GAP
            : 0;
        //
        const extraPaddingForTracksAroundL = s.SHOW_TILES && c.tileBreakL
            ? c.hasTracksAroundL
                ? s.TILE_HORIZONTAL_PADDING
                : 0
            : c.hasTracksAroundL && c.hasTracksNormalL
                ? s.EDGE_TO_EDGE_HORIZONTAL_GAP
                : 0;
        const widthTracksAroundL = c.hasTracksAroundL
            ? (c.numberTracksAroundL - 1) * s.EDGE_TO_EDGE_HORIZONTAL_GAP
            : 0;
        //
        const onlyHalfSpaceL = i > 0 && !c.hasTracksNormalL && !c.tileBreakL && !c.hasTracksAroundL;
        const onlyHalfSpaceR = i < maxIndex &&
            !c.hasTracksNormalR &&
            !c.tileBreakR &&
            !c.hasTracksAroundR;
        const paddingL = onlyHalfSpaceL
            ? s.TILE_HORIZONTAL_PADDING / 2
            : s.TILE_HORIZONTAL_PADDING;
        const paddingR = onlyHalfSpaceR
            ? s.TILE_HORIZONTAL_PADDING / 2
            : s.TILE_HORIZONTAL_PADDING;
        //
        c.xl = current;
        ...
        c.nodeL =
            current + extraPaddingForTracksAroundL + widthTracksAroundL + paddingL;
        c.xc =
            current +
                extraPaddingForTracksAroundL +
                widthTracksAroundL +
                paddingL +
                c.maxNodeWidth / 2;
        c.nodeR =
        ...
        current +=
            extraPaddingForTracksAroundL +
                ...
                c.maxNodeWidth +
                paddingR +
                (widthTracksNormalR + extraPaddingForTracksNormalR + horizontalTileGapR);
    });
    ...
    columns.forEach((colInfo, i) => {
        colInfo.nodeIds.forEach((nodeId) => {
            ...
            nodeCoordMap[nodeId].x =
                columns[nodeLayer].xc - nodeCoordMap[nodeId].w / 2;
