...
export function setKeyColors(kc) {
    _KEY_COLORS.clear();
...
export function getColorAsRgb(colorKey) {
    if (colorKey === "none") {
...
export function generateKeyColors(color, nForContrast, whiteStartIndex, blackStartIndex) {
    const n = nForContrast;
    const iw = whiteStartIndex;
    const ib = blackStartIndex;
    assert(n > iw + 2 && n > ib + 4, "nForContrast must be higher to allow for start indexes");
    ...
    console.log("%cbaseContentLessVisible XX", `color: ${baseContentLessVisible}`);
