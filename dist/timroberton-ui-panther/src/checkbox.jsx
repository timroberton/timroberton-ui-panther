    ...
    return (<div class="">
      <label class="inline-flex items-center align-top">
        <div class="relative flex-none w-5 h-5 mr-2">
          <input checked={p.checked} type="checkbox" onChange={(v) => p.onChange(v.currentTarget.checked)} class="peer ui-focusable appearance-none h-5 w-5 cursor-pointer rounded border border-base-300 bg-base-100 text-base-content"/>
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 absolute inset-0.5 pointer-events-none peer-checked:block hidden text-base-content" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round">
...
export function RadioGroup(p) {
    return (<div class="">
      <Show when={p.label} keyed>
        {(keyedLabel) => {
            return (<legend class="pb-1 text-sm text-base-content inline-block">
              {keyedLabel}
            </legend>);
        }}
      </Show>
